class Usuario < ActiveRecord::Base
  ...
  validates_inclusion_of :perfil, :in => %w( ADMIN USUARIO ), :if => :perfil_con_datos?, :message => "Perfil inválido. Valores válidos son: ADMIN, USUARIO"
  validates_format_of :email, :with => /^([^@\s]+)@((?:[-a-z0-9]+\.)+[a-z]{2,})$/i, :if => :email_con_datos?, :message => 'Formato de e-mail inválido'
