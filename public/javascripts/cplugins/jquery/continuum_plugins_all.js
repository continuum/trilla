 ...
 * jquery.format.js
 *
 * jQuery Continum Utils Format plugins - v1.0
 * http://continuum.cl/
 *
 * Copyright (c) 2009 Continuum
 * @author victor.utreras	
 */

(function ($) {
		
	/**
	 * aplica el formato de rut chileno a un rut. 
	 * 	Ej: 
	 * 		input: 13872123 	-> 13.872.123-4
	 * 		input: 13872123-4 	-> 13.872.123-4
	 */
	$.formatRut = function (rut) {
		
		var rutArray = rut.toString();
		var rr = rutArray.split('-');
		var r = rr[0];
		var dv = '';
		...
		return out.reverse() + '-' + dv;
	};	
	
	/**
	 * 
	 */
	$.getDigitoVerificador = function (rut) {
	  var mult = 2, sum = 0;
	  for (var n = rut.length; n > 0; n--) {
	    sum += (mult * rut.substring(n-1, n));
	    mult++;
	    if (mult == "8") {
	      mult = 2;
	    }
	  ...
	  var resto = (11 - (sum % 11)) % 11;
	  return resto == 10 ? "K" : resto;
	}

	/**
	 * 
	 */
	$.validaRutDv = function (rut, dv) {
	  if (dv == "k") {
	    dv = "K";
	  }
	  
	  return $.getDigitoVerificador(rut) == dv;
	}
	
	/**
	 * agrega formato por clase
	 */
	$.fn.applyFormatByClass = function() {
		
		var el = $(this);
		var valor = el.val();

		if (el.hasClass('format-rut')) {
			el.setVal($.formatRut(valor.trim()));
		}
	...
	/**
	 * agrega formato por clase al formulario
	 */
	$.fn.applyFormatByClassForm = function () {
		
		var el = $(this);
		
		el.find(':text').each(function (i, e){
			$(e).applyFormatByClass();
		});
	  ...
	  */
	 $.redondearDecimales = function (nD, nDec) 	{  
...
(function ($) {
	 ...
	 * 
	 */
	function JQPagina(pagina_, counter_, totalPaginas_, count_){
		
		this.pagina = pagina_;
		this.counter = counter_;
		this.totalPaginas = totalPaginas_;
		this.count = count_;
	}
	 ...
	 * 
	 */
	function calcularTotalPaginas(count, registros) {
		var n = Number(count) / registros;
		//var totalPaginas = n.toString().indexOf('.') != -1 ? Math.round(n) : Math.round(n);
		var totalPaginas = Math.round(n);
		if ((registros * totalPaginas) < count) totalPaginas++; 
		return totalPaginas < 0 ? 1 : totalPaginas;
	}
	
	/**
	 * 
	 */
	function calculaDesdeHasta(pag) {
		
		var hasta = pag.counter * pag.pagina;
		var desde = (hasta - pag.counter) + 1;
		var total = pag.count;
		...
		desde = desde > total ? total : desde;
		hasta = hasta > total ? total : hasta;
	 ...
	 * 
	 */
	function creaPaginaPublica(pag) {
		 
		var dh = calculaDesdeHasta(pag);
		pag.totalPaginas = calcularTotalPaginas(dh.total, pag.counter);
		
		return {registrosPorPagina: pag.counter, numeroPagina: pag.pagina, desde: dh.desde, hasta: dh.hasta, total: dh.total, totalPaginas: pag.totalPaginas};
	}
	
	/**
	 * 
	 */
	function refreshPages(p, paginaPublica) {
		p.find('.a-pager').each(function (i, e) {
			if (i+1 > paginaPublica.totalPaginas) {
				$(e).hide();
			} else {
				$(e).show();
			}
	...
	/**
	 * plugin - contruye el pagerCount
	 */
	$.fn.pagesCount = function(callback, pag, pFooter) {
		
		var el = $(this).empty();
		
		if (pag.totalPaginas <= 0) return el;
		
		el.append('<p>Resultados por p&aacute;gina: </p>').append(
				'<ul>' + 
		    	'	<li><a href="javascript:void(0);" class="a-pager-count" title="5">5</a></li>' + 
		    	'	<li><a href="javascript:void(0);" class="a-pager-count" title="10">10</a></li>' + 
		    	'	<li><a href="javascript:void(0);" class="a-pager-count" title="20">20</a></li>' + 
		    	'	<li><a href="javascript:void(0);" class="a-pager-count" title="-1">Todos</a></li>' + 
		   		'</ul>');
		
		el.find('a[title=' + pag.counter + ']').addClass('selectPage');
		
		if (callback && (typeof callback === 'function')) {
			
			el.find('.a-pager-count').unbind('click').click(function (event) {
				
				el.find('a[title=' + pag.counter + ']').removeClass('selectPage');
				
				pag.counter = $(this).text();
				pag.counter = pag.counter === 'Todos' ? -1 : Number(pag.counter);
				pag.pagina = 1;
				
				if (pFooter){
					pag.count = Number(pFooter.attr('title'));
				}
				
				el.find('a[title=' + pag.counter + ']').addClass('selectPage');
				
				var paginaPublica = creaPaginaPublica(pag);
				
				callback(paginaPublica);
				
				if (pFooter.callback_click) {
					pFooter.callback_click(paginaPublica);
				}
	...
	/**
	 * plugin - contruye el pager
	 */
	$.fn.pages = function(callback, pag, pFooter) {
		...
		for (var i = 1; i <= pag.totalPaginas; i++) {
			s+=' <li><a href="javascript:void(0);" class="a-pager" title="' + i + '">' + i +'</a></li>';
		}
		
		s+='</ul>';
						
		el.append('<p>P&aacute;gina: </p>').append(s);
		
		el.find('a[title=' + pag.pagina + ']').addClass('selectPage');
		
		if (callback && (typeof callback === 'function')) {
			
			el.find('.a-pager').unbind('click').click(function (event) {
				
				el.find('a[title=' + pag.pagina + ']').removeClass('selectPage');
				
				pag.pagina = Number($(this).text());
	...
	/**
	 * @public - construye el pagerFooter formado por: pager y pagerCount
	 */
	$.fn.pagerFooter = function(options) {
		
		var el = $(this);
		var id = el.attr('id');

		var pCount = $('<div id="' + id + '-pager-count" class="secondPaginator">');
		var p = $('<div id="' + id + '-pager" class="masterPaginator">');
		
		el.count = function(count_) {

			var c = el.attr('title');
			
			if (count_) {

				if (c == undefined || Number(c) != Number(count_)) {
				
					var pag = new JQPagina(1, 5, calcularTotalPaginas(count_, 5), count_);
					if(pag.totalPaginas != 1) {
						pCount.pagesCount(options.selectEvent, pag, el);
						p.pages(options.selectEvent, pag, el);
					}
					
					el.attr('title',count_);
					
					var paginaPublica = creaPaginaPublica(pag);
					el.callback_click(paginaPublica);
				}
...
//				var count2_ = count_;
//				
//				var pag = new JQPagina(1, 5, calcularTotalPaginas(count2_, 5), count2_);
//				
//				pCount.pagesCount(options.selectEvent, pag, el);
//				p.pages(options.selectEvent, pag, el);
//				
//				el.attr('title',count2_);
//				var paginaPublica = creaPaginaPublica(pag);
//				el.callback_click(paginaPublica);
		...
		el.callback_click = function(paginaPublica) {
			if (options.pagerHeader) {
				refreshPages(p, paginaPublica);
				options.pagerHeader.refresh(paginaPublica);
			}
		};

		return el.append(p).append(pCount);
	}	
	
	/**
	 * @public - oculta el pagerCount
	 */
	$.fn.hidePagerCount = function() {
		var el = $(this);
		var id = el.attr('id');
		el.find('#' + id + '-pager-count').hide();
		$('.paginatorStandar').hide();
		return el;
	}
	
	/**
	 * @public - muestra el pagerCount
	 */
	$.fn.showPagerCount = function() {
		var el = $(this);
		var id = el.attr('id');
		$('.paginatorStandar').show();
		el.find('#' + id + '-pager-count').show();
		return el;
	}
	
	/**
	 * @public - oculta el pager
	 */
	$.fn.hidePager = function() {
		var el = $(this);
		var id = el.attr('id');
		el.find('#' + id + '-pager').hide();
		$('.paginatorStandar').hide();
		return el;
	}
	 ...
	 * @public - muestra el pagerCount
	 */
	$.fn.showPager = function() {
		var el = $(this);
		var id = el.attr('id');
		$('.paginatorStandar').show();
		el.find('#' + id + '-pager').show();
		return el;	
	}
	
	/**
	 * @public - construye el pagerHeader
	 */
	$.fn.pagerHeader = function (titulo) {
	
		var el = $(this);
		var id = el.attr('id');
	
		el.refresh = function(paginaPublica) {
			
			this.empty().append('<span>' + titulo + ' del </span>' + 
					'<span><strong>' + paginaPublica.desde + '</strong></span>' + 
					'<span> a </span>' + 
					'<span><strong>' + paginaPublica.hasta + '</strong></span>' + 
					'<span> de </span>' + 
					'<span><strong>' + paginaPublica.total + '</strong></span>');
		}
		
		var count = el.attr('title');
		if (count) {
			var pag = new JQPagina(1, 5, calcularTotalPaginas(count, 5), count);
			var paginaPublica = creaPaginaPublica(pag);
			el.refresh(paginaPublica);
		}
		
		return el;
	}
	
	var PAGER_DEFAULT = {registrosPorPagina: 5, numeroPagina: 1, desde: 1, hasta: 5, total: 0, totalPaginas: 0};
	
	/**
	 * 
	 */
	$.getPagerDefault = function() {
		return $.extend({}, PAGER_DEFAULT);
	}
	
	/**
	 * 
	 */
	$.setPagerDefault = function(pager) {
		var p = {registrosPorPagina: 5, numeroPagina: 1, desde: 1, hasta: 5, total: 0, totalPaginas: 0};
		p = $.extend(p, pager);
		PAGER_DEFAULT = p;
	}
...
(function ($) {
	...
	/**
	 * llena los campos con valores mock
	 */
	$.fillFields = function (selector, options) {
		
		var n = $.numeroRandom(2,100);
			...
			if (e.type === 'text' || e.type === 'textarea') {
				try {
					...
					if (el.hasClass('number')) {
				        el.setVal($.numeroRandom(2,20));
				    } else if (el.hasClass('mail')) {
				    	el.setVal('mail_' + n + '@continuum.cl');
				    } else if (el.hasClass('phone')) {
				    	el.setVal('02-36782' + n);	
				    } else if (el.hasClass('date')) {
				    	var dia = $.numeroRandom(1,28);
				    	var mes = $.numeroRandom(1,12);
				    	el.setVal(dia+'/' + mes + '/2010');	
				    } else {
				        el.setVal(el.attr('id'));
				        //el.setVal('++-~� \\ \/\u00f1 \u00d1 \u00e1 \u00e9 \u00ed \u00f3 \u00fa \u00fe <b>' + el.attr('id') + "</b>\n Salto l�nea(<-acento)comillas:\'simple\"doble" + n);
				    }
				    
				} catch(e) {
					el.text('++-~� \\ \/\u00f1 \u00d1 \u00e1 \u00e9 \u00ed \u00f3 \u00fa \u00fe <b>' + el.attr('id') + "</b>\n Salto l�nea(<-acento)comillas:\'simple\"doble" + n);
				}
			} else if (e.type === 'select-one') {
				
				if (options && options.select && options.select.minValue && options.select.maxValue) {
					el.attr('selectedIndex', $.numeroRandom(options.select.minValue, options.select.maxValue));
				}
			}
